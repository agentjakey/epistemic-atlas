...
The workflow converts raw source material into a structured atlas entry. The default order has seven stages: stages 1 through 6 build the entry, and stage 7 is adversarial, actively trying to find errors in what the earlier stages built. The order is a starting path, not a strict one-way pipeline. Work in real disputes is iterative, and the schema is built to support that (see the note on a living workflow below).
...
**Stage 2 (Source ingestion):** Build source objects with full provenance metadata. All consulted sources are retained regardless of quality or position. Missing provenance is set to null, not invented. Conflicts of interest are recorded when known.

**Stage 3 (Claim extraction):** Extract every significant epistemic claim as an ExtractedClaim object with verbatim or near-verbatim text, staying close to the source. Each extracted claim belongs to exactly one source. Extraction does not normalize, relate, or assess.

**Stage 4 (Claim normalization):** Group extracted claims into NormalizedClaim objects: unambiguous, scope-explicit propositions. The extracted_claim_ids field preserves the link to the source material. A single normalized claim can synthesize evidence from multiple extracted claims and sources.

**Stage 5 (Relation mapping):** Build a directed relation graph between normalized claims using five relation families (supports, opposes, depends_on, contextualizes, equivalent), with optional subtype and tags for nuance and a basis field recording how each link is grounded. Relations are part of the core knowledge structure; cruxes, flags, and assessment do not belong here.

**Stage 6 (Assessment layer):** Build the interpretive layer on top of the core structure. Identify 2 to 7 pivotal questions as Crux objects with resolution status and dependency links. Attach FailureModeFlag objects to specific normalized claims, sources, or relations using a twelve-type controlled vocabulary, applied symmetrically across all positions in the dispute. Record missing evidence. Then synthesize one or more Assessment objects: status (settled, unsettled, or partially_settled), settled_direction if settled, key crux IDs, weak link IDs, dominant failure modes, and explicit what-would-update scenarios. Cruxes, flags, missing evidence, and assessments all live in the assessment_layer, separate from the core relation graph, and each assessment must be internally consistent with the claim and crux-level data.

**Stage 7 (Adversarial audit):** Actively review the completed entry for normalization drift, asymmetric failure flagging, inconsistent crux identification, and hallucinated source references if the entry was LLM-assisted. Record unresolved issues as AuditNote objects. A known open issue is better than a silent inaccuracy.

**A living workflow, not a one-way pipeline.** The seven stages describe the default order, but a finished entry is rarely the end. A newly identified crux, a missing-evidence item, an audit note, or a newly found source can each send the work back to an earlier stage: rescope the question, reingest sources, re-extract claims, renormalize, remap relations, or reassess. The schema records these as trigger fields on cruxes, missing-evidence items, and audit notes (rescope, reingest, reextract, renormalize, remap_relations, reassess, re_review), so the next person to pick up an entry can see what it is asking to have revisited. This is a design affordance for iterative work. The orchestration around it (tools that act on triggers automatically) is still early, and the triggers are best read as hand-set pointers rather than an automated engine.
...
*schema/epistemic-atlas.schema.json, schema/GUIDE.md, schema/examples/, data/lhc/, data/eggs/, prompts/ (prompt templates for the workflow stages), lib/types_v3.ts (TypeScript types), the interactive Next.js prototype (app/), the companion documents docs/methodology.md, docs/limitations.md, and docs/judging_alignment.md, and the live-systems compute-scaling validation in experiments/compute_scaling/ (a smoke-stage run of the workflow under a single-pass and a six-stage condition on a frozen source packet, with deterministic evaluation code and schema-valid exemplar outputs). The compute-scaling work is smoke-stage evidence that the measurement pipeline runs and that both conditions produce schema-valid, source-linked graphs; it is not a comparison of the conditions and makes no superiority claim.*
